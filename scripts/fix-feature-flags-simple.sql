-- 🔧 Fix Feature Flags Function - Simple Version (No Overloads)
...
BEGIN
    ...
    DROP FUNCTION IF EXISTS get_feature_flag_v3(TEXT, UUID, TEXT);
...
-- 3. Create hashint8 helper function
...
-- 4. Create the main function with a unique name
CREATE OR REPLACE FUNCTION get_feature_flag_simple(
  p_flag_key TEXT,
...
-- 5. Create convenience functions with different names
CREATE OR REPLACE FUNCTION get_feature_flag_global(p_flag_key TEXT)
RETURNS JSONB AS $$
BEGIN
  RETURN get_feature_flag_simple(p_flag_key, NULL, 'production');
END;
$$ LANGUAGE plpgsql SECURITY DEFINER;

CREATE OR REPLACE FUNCTION get_feature_flag_user(p_flag_key TEXT, p_user_id UUID)
RETURNS JSONB AS $$
BEGIN
  RETURN get_feature_flag_simple(p_flag_key, p_user_id, 'production');
END;
$$ LANGUAGE plpgsql SECURITY DEFINER;

CREATE OR REPLACE FUNCTION get_feature_flag_full(p_flag_key TEXT, p_user_id UUID, p_environment TEXT)
RETURNS JSONB AS $$
BEGIN
  RETURN get_feature_flag_simple(p_flag_key, p_user_id, p_environment);
END;
$$ LANGUAGE plpgsql SECURITY DEFINER;

-- 6. Create the main get_feature_flag function that uses the simple version
...
-- Available functions:
-- get_feature_flag(p_flag_key) - Simple global flag check
-- get_feature_flag_global(p_flag_key) - Global flag check
-- get_feature_flag_user(p_flag_key, user_id) - User-specific check
-- get_feature_flag_full(p_flag_key, user_id, environment) - Full check
-- get_feature_flag_simple(p_flag_key, user_id, environment) - Core function
