-- 🔧 Fix Functions by Dropping and Recreating
-- This script DROPS existing functions first, then recreates them with correct signatures
...
DROP FUNCTION IF EXISTS get_feature_flag(UUID, TEXT, TEXT);
DROP FUNCTION IF EXISTS get_feature_flag(UUID, TEXT);
DROP FUNCTION IF EXISTS get_feature_flag(UUID);

DROP FUNCTION IF EXISTS update_cache(TEXT, JSONB, INTEGER);
DROP FUNCTION IF EXISTS update_cache(TEXT, JSONB);
DROP FUNCTION IF EXISTS update_cache(TEXT, TEXT, INTEGER);
DROP FUNCTION IF EXISTS update_cache(TEXT, TEXT);

DROP FUNCTION IF EXISTS get_cache(TEXT);
DROP FUNCTION IF EXISTS get_cache(UUID);

DROP FUNCTION IF EXISTS clear_cache(TEXT);
DROP FUNCTION IF EXISTS clear_cache(UUID);
DROP FUNCTION IF EXISTS clear_cache();

DROP FUNCTION IF EXISTS set_cache(TEXT, JSONB, INTEGER);
DROP FUNCTION IF EXISTS set_cache(TEXT, JSONB);
DROP FUNCTION IF EXISTS set_cache(TEXT, TEXT, INTEGER);
DROP FUNCTION IF EXISTS set_cache(TEXT, TEXT);

DROP FUNCTION IF EXISTS hashint8(TEXT);
DROP FUNCTION IF EXISTS hashint8(UUID);

-- 2. CREATE FUNCTIONS WITH CORRECT SIGNATURES

-- Create get_feature_flag function
...
-- Create set_cache alias for backward compatibility
...
-- ✅ Functions dropped and recreated successfully!
