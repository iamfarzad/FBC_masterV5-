...
-- This migration creates all necessary tables and optimizations from scratch

-- 1. Create lead_summaries table first (required by other tables)
...
-- 2. Create lead_search_results table
...
-- 4. Create token_usage_logs table
...
-- 7. Create performance indexes for lead_summaries
...
-- 8. Create performance indexes for lead_search_results
...
-- 9. Create performance indexes for activities
CREATE INDEX IF NOT EXISTS idx_activities_type ON activities(type);
CREATE INDEX IF NOT EXISTS idx_activities_status ON activities(status);
CREATE INDEX IF NOT EXISTS idx_activities_created_at ON activities(created_at DESC);

-- 10. Create performance indexes for token_usage_logs
...
-- 12. Create performance indexes for user_budgets
CREATE INDEX IF NOT EXISTS idx_user_budgets_user_id ON user_budgets(user_id);

-- 13. Create performance indexes for ai_responses
...
-- 15. Create RLS policies for lead_summaries
CREATE POLICY "lead_summaries_select_own" ON lead_summaries
  FOR SELECT USING (auth.uid() = user_id);

CREATE POLICY "lead_summaries_insert_own" ON lead_summaries
  FOR INSERT WITH CHECK (auth.uid() = user_id);

CREATE POLICY "lead_summaries_update_own" ON lead_summaries
  FOR UPDATE USING (auth.uid() = user_id);

CREATE POLICY "lead_summaries_delete_own" ON lead_summaries
  FOR DELETE USING (auth.uid() = user_id);

-- 16. Create RLS policies for lead_search_results
CREATE POLICY "lead_search_results_select_own" ON lead_search_results
  FOR SELECT USING (
    EXISTS (
...
CREATE POLICY "lead_search_results_insert_own" ON lead_search_results
...
-- 17. Create RLS policies for activities
CREATE POLICY "activities_select_public" ON activities
  FOR SELECT USING (true);

CREATE POLICY "activities_insert_public" ON activities
  FOR INSERT WITH CHECK (true);

-- 18. Create RLS policies for token_usage_logs
...
-- 19. Create RLS policies for user_budgets
...
-- 20. Create RLS policies for ai_responses
...
-- 22. Create performance monitoring functions
...
-- 23. Create database health check function
...
-- 24. Create triggers for updated_at columns
...
GRANT SELECT, INSERT, UPDATE, DELETE ON lead_summaries TO authenticated;
GRANT SELECT, INSERT ON lead_search_results TO authenticated;
...
-- 26. Insert default budgets for existing users
...
-- 28. Analyze tables for better query planning
...
SELECT 'Complete database setup migration completed successfully' as status;
