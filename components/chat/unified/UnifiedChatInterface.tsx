...
import { Send, RotateCcw, Settings, Maximize2, Copy, Check, Edit, Languages, User, Mic, Phone, Download, BookOpen, FileText, Camera, Monitor, Calculator, Video, Code, Plus, X } from 'lucide-react'
import { 
  Conversation, 
  ConversationContent, 
...
export const StatusBar = memo<{ isLoading: boolean; currentStage?: number; totalStages?: number }>(({ isLoading, currentStage = 1, totalStages = 7 }) => {
  const progress = ((currentStage - 1) / (totalStages - 1)) * 100

  return (
    <div className="flex-shrink-0 border-t border-border bg-background/95 px-4 py-2 text-xs">
...
export const EnhancedHeader = memo<{
  onReset: () => void
  onSettings: () => void
  sessionId?: string | null
}>(({ onReset, onSettings, sessionId }) => {
  return (
