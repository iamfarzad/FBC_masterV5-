 ...
 */
export function generatePdfPath(sessionId: string, name: string): string {
  const sanitizedName = name.replace(/[^a-zA-Z0-9]/g, '_').toLowerCase();
  const timestamp = new Date().toISOString().slice(0, 19).replace(/:/g, '-');
  return path.join(process.cwd(), 'generated-pdfs', `${sessionId}_${sanitizedName}_${timestamp}.pdf`);
}
